...
export function ProjectPage({ projectId, initialTab = "documents" }: Props) {
    ...
    function renderFolderInput(parentId: string | null, depth: number) {
        ...
        return (
            <div
                ...
                className="group flex items-center h-10 pr-8 border-b border-border/40"
                key={`new-folder-${parentId ?? "root"}`}
            >
                <div
                    className={`sticky left-0 z-[60] ${CHECK_W} bg-background p-2 flex items-center justify-center self-stretch`}
                    style={treeControlCellStyle(depth)}
                >
                    <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/50 shrink-0" />
                </div>
                <div
                    className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-background p-2`}
                ...
                >
                    <div className="flex items-center gap-1.5">
                        ...
                        <input
                            autoFocus
                            className="flex-1 min-w-0 text-sm text-foreground/90 bg-transparent outline-none border-b border-border"
            ...
            >
                ...
                >
                    <input
                        type="checkbox"
                        disabled
                        className="h-2.5 w-2.5 rounded border-border cursor-default accent-black disabled:opacity-100"
                    />
                </div>
                <div
                    className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-background p-2`}
                ...
                <div className="ml-auto w-20 shrink-0 text-xs text-muted-foreground/50 uppercase truncate">
                    {filename.includes(".") ? filename.split(".").pop() : "file"}
                </div>
                <div className="w-24 shrink-0 text-sm text-muted-foreground/50">
                    Uploading
                </div>
                            ...
                            >
                                {(() => {
                                    ...
                                    return (
                                        <>
                                <div
                                    className={`sticky left-0 z-[60] ${CHECK_W} p-2 flex items-center justify-center ${rowBg} group-hover:bg-muted/50`}
                                    style={treeControlCellStyle(depth)}
                                ...
                                <div className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-background p-2 group-hover:bg-muted/50`} style={treeNameCellStyle(depth)}>
                                <div className="flex items-center gap-2">
                                    {isProcessing ? (
                                        <Loader2 className="h-4 w-4 animate-spin text-muted-foreground/70 shrink-0" />
                                    ) : isError ? (
                                    ...
                                    {renamingDocumentId === doc.id ? (
                                        <input
                                            autoFocus
                                            className="min-w-0 flex-1 text-sm text-foreground/90 bg-transparent outline-none border-b border-border"
                                        ...
                                        <span className="text-sm text-foreground/90 truncate">{doc.filename}</span>
                                    )}
                                </div>
                                </div>
                    ...
                    return (
                        <div key={`folder-${folder.id}`}>
                            <div
                                ...
                                className={`group flex items-center h-10 pr-8 border-b border-border/40 hover:bg-muted/50 cursor-pointer transition-colors ${isRenaming ? "" : "select-none"} ${dragOverFolderId === folder.id ? "bg-blue-50 ring-1 ring-inset ring-blue-200" : ""}`}
                            >
                                <div className={`sticky left-0 z-[60] ${CHECK_W} p-2 flex items-center justify-center ${dragOverFolderId === folder.id ? "bg-blue-50" : "bg-background"} group-hover:bg-muted/50 self-stretch`} style={treeControlCellStyle(depth)}>
                                    {isExpanded
                                ...
                                <div className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} p-2 ${dragOverFolderId === folder.id ? "bg-blue-50" : "bg-background"} group-hover:bg-muted/50`} style={treeNameCellStyle(depth)}>
                                <div className="flex items-center gap-1.5">
                                    {isExpanded
                                    ...
                                    {isRenaming ? (
                                        <input
                                            autoFocus
                                            className="flex-1 min-w-0 text-sm text-foreground/90 bg-transparent outline-none"
    ...
    const actionsDropdown = currentSelectionCount > 0 ? (
        <div ref={actionsRef} className="relative">
            ...
            {actionsOpen && (
                <div className="absolute top-full right-0 mt-1 w-36 rounded-lg border border-border/60 bg-card shadow-lg z-[120] overflow-hidden">
                    {tab === "documents" && (
                        <button
            ...
            {tab === "documents" && (
                <>
                    <button
                        ...
                        className="flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground/80 transition-colors"
                    >
                ...
                {tab === "documents" && (
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex items-center h-8 pr-8 border-b border-border text-xs text-muted-foreground font-medium select-none shrink-0">
                            ...
                            <div className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-card pl-2 text-left`}>
                                Name
                            </div>
                            ...
                            >
                                ...
                                {q ? (
                                    ...
                                    setViewingDoc(doc);
                                                    ...
                                                    className="group flex items-center h-10 pr-8 border-b border-border/40 hover:bg-muted/50 cursor-pointer transition-colors"
                                                >
                                                    <div className={`sticky left-0 z-[60] ${CHECK_W} p-2 flex items-center justify-center ${selectedDocIds.includes(doc.id) ? "bg-muted/50" : "bg-background"} group-hover:bg-muted/50`} onClick={(e) => e.stopPropagation()}>
                                                        <input
                                                            type="checkbox"
                                                            checked={selectedDocIds.includes(doc.id)}
                                                    ...
                                                    <div className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-background p-2 group-hover:bg-muted/50`}>
                                                    <div className="flex items-center gap-2">
                        ...
                        {contextMenu &&
                            (() => {
                                ...
                                return (
                                    <div
                                        ref={contextMenuRef}
                                        className="fixed z-[120] w-48 rounded-xl border border-border/60 bg-card shadow-lg overflow-hidden"

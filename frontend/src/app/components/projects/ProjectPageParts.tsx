...
export function DocVersionHistory({
    ...
    if (loading && versions.length === 0) {
        return (
            <div className="flex items-center h-9 border-b border-border/40 text-xs text-muted-foreground bg-muted/60">
                <div
    ...
    if (versions.length === 0) {
        return (
            <div className="flex items-center h-9 border-b border-border/40 text-xs text-muted-foreground/70 bg-muted/60">
                <div
                    className={`sticky left-0 z-[60] ${CHECK_W} bg-muted/60 self-stretch`}
                    style={treeControlCellStyle(depth)}
                />
            ...
            {ordered.map((v) => {
                ...
                return (
                    <div
                        ...
                        className="group flex items-center h-9 pr-3 md:pr-10 border-b border-border/40 bg-muted/60 text-xs text-muted-foreground cursor-pointer hover:bg-gray-100/80 transition-colors"
                    >
                        <div
                            className={`sticky left-0 z-[60] ${CHECK_W} bg-muted/60 group-hover:bg-gray-100/80 self-stretch`}
                            style={treeControlCellStyle(depth)}
                        />
                        <div
                            className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-muted/60 group-hover:bg-gray-100/80 p-2`}
                            style={treeNameCellStyle(depth)}
                        >
                            <div className="flex items-center gap-2">
                                ...
                                {isEditing ? (
                                    <input
                                        ...
                                        className="min-w-0 flex-1 max-w-[240px] border-b border-border bg-transparent text-xs text-foreground/90 outline-none focus:border-border/400"
                                    />
                                ) : (
                                ...
                                {!isEditing && onRenameVersion && (
                                    <button
                                        ...
                                        className="shrink-0 rounded p-0.5 text-muted-foreground/70 opacity-0 group-hover:opacity-100 hover:text-foreground/80 hover:bg-border transition"
                                    >
                        ...
                        <div className="w-8 shrink-0 flex justify-end">
                            <button
                                ...
                                className="flex items-center justify-center w-6 h-6 rounded text-muted-foreground hover:text-foreground/90 hover:bg-accent transition-colors"
                            >
        ...
        <div className="flex-1 overflow-y-auto bg-background">
            <div className="mb-1 flex items-start justify-between px-4 py-3 md:px-10">
                ...
                <div className="flex items-center gap-4">
                    <div className="h-8 w-8 rounded bg-accent animate-pulse" />
                    <div className="h-8 w-8 rounded bg-accent animate-pulse" />
                    <div className="h-8 w-11 rounded bg-accent animate-pulse" />
                    <div className="h-8 w-28 rounded bg-accent animate-pulse" />
                </div>
            </div>
            <div className="flex items-center h-10 px-4 md:px-10 border-b border-border gap-5">
            ...
            <div className="flex items-center h-8 pr-3 md:pr-10 border-b border-border">
                <div className="w-8 shrink-0" />
                <div className="flex-1 min-w-0 pl-3 pr-4">
                    <div className="h-2.5 w-8 rounded bg-accent animate-pulse" />
                </div>
            ...
            {[1, 2, 3, 4, 5].map((i) => (
                <div
                    key={i}
                    className="flex items-center h-10 pr-3 md:pr-10 border-b border-border/40"
                >
                    <div className="w-8 shrink-0" />
                    <div className="flex-1 min-w-0 pl-3 pr-4">
                        <div className="h-3.5 w-56 rounded bg-accent animate-pulse" />
                    </div>
                    <div className="w-20 shrink-0">

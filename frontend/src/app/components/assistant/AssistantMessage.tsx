...
import { Copy, Check, ChevronDown, Download, Loader2, Volume2, Square } from "lucide-react";
            ...
            for (const { annotation } of pending) {
                ...
                try {
                    const resp = await fetch(
                        `/api/single-documents/${annotation.document_id}/edits/${annotation.edit_id}/${verb}`,
                        {
                            method: "POST",
                            headers: token
    ...
    return (
        <div className="flex items-center gap-2">
            <button
                ...
                disabled={!!busy}
                className="px-2 py-1 text-xs rounded border border-foreground bg-foreground text-white hover:bg-gray-800 disabled:opacity-50 inline-flex items-center gap-1"
            >
                {busy === "accept" && (
            ...
            {onViewClick && first && (
                <button
                    ...
                    disabled={!!busy}
                    className="ml-auto px-2 py-1 text-xs rounded border border-border bg-card text-foreground/80 hover:bg-accent disabled:opacity-50"
                >
                    View
                </button>
            )}
        ...
        <div className="border border-border rounded-lg bg-card overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 pt-3">
                ...
                <button
                    ...
                    className="shrink-0 rounded p-1 text-muted-foreground hover:bg-accent hover:text-foreground/90 transition-colors"
                >
                    <ChevronDown
...
function ResponseStatus({ status }: { status: StatusState }) {
    ...
    const isDone = showDone && doneVisible;
        ...
        <div className="w-full h-9 flex items-center mb-2">
            <span className="relative shrink-0 inline-block">
                <img
                    ...
                    width={22}
                    height={22}
                    className={`rounded-sm ${isActive ? "animate-spin" : ""}`}
                    style={{ animationDuration: "3s" }}
                />
                {isDone && (
                    <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-green-500 border border-background" />
                )}
                {isError && (
                    <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-destructive border border-background" />
                )}
            </span>
        </div>
    );
...
function DocDownloadBlock({
    ...
    const href = isSafeHref ? download_url : null;
    ...
    const downloadIcon = spinning ? (
        ...
        <button
            ...
            className="shrink-0 flex items-center border-l border-border px-6 bg-card text-muted-foreground/70 hover:bg-accent hover:text-muted-foreground transition-colors cursor-pointer"
        >
            <Download size={13} />
        </button>
    );
        ...
        >
            <ReactMarkdown
                ...
                components={{
                    ...
                    tbody: ({ node, ...props }) => (
                        <tbody
                            className="divide-y divide-gray-200 bg-card"
                            {...props}
                        />
                    ),
                    ...
                    code: ({ node, children, ...props }) => {
                            ...
                            if (annotation) {
                                ...
                                return (
                                    <button
                                        ...
                                        className="mx-0.5 inline-flex items-center justify-center rounded-full w-4 h-4 text-[10px] font-medium transition-colors align-super bg-accent text-foreground hover:bg-border"
                                        title={tooltipText}
                                    >
                                        {idx + 1}
                                    </button>
                        ...
                        return (
                            <code
                                className="bg-accent px-1.5 py-0.5 rounded text-sm font-serif"
                                {...props}
                            >
                                {children}
                            </code>
...
function extractTtsText(events: AssistantEvent[], fallback: string): string {
    const fromEvents = events
        .filter((e) => e.type === "content")
        .map((e) => (e as Extract<AssistantEvent, { type: "content" }>).text)
        .join("\n\n")
        .trim();
    return stripMarkdownForTts(fromEvents || fallback);
}
...
function TtsButton({ text }: { text: string }) {
    ...
    const handleClick = async () => {
        ...
        try {
            ...
            const token = session?.access_token;
            const voiceId = localStorage.getItem("lanti-voice-id") ?? undefined;
            const speed = parseFloat(localStorage.getItem("lanti-voice-speed") ?? "1.0");
...
export function AssistantMessage({
    ...
    const ttsText = extractTtsText(events ?? [], _content);
                ...
                <div className="flex items-center gap-2 pt-2 pb-4 md:pb-8 font-sans justify-start">
                    {!isStreaming && (
                        <>
                            ...
                            <button
                                className="p-1.5 rounded text-muted-foreground hover:text-foreground/80 hover:bg-accent"
                                onClick={handleCopy}
                            >
                                {isCopied ? (

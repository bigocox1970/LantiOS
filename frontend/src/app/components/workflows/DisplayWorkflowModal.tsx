...
function Toggle({ on, onToggle }: { on: boolean; onToggle: () => void }) {
    return (
        <button
            type="button"
            onClick={onToggle}
            className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors duration-200 ${on ? "bg-foreground" : "bg-border"}`}
        >
            <span
    ...
    return (
        <div className="relative">
            <input
                ...
                className="w-full text-xs text-foreground/80 placeholder:text-muted-foreground/70 bg-muted/50 border border-border rounded-md px-3 py-2 outline-none"
            />
            {selectedId && (
                <button
                    onMouseDown={() => {
                    ...
                    {filtered.length === 0 ? (
                        ...
                        filtered.map((p) => (
                            <button
                                ...
                                className="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-left hover:bg-muted/50 text-foreground/80"
                            >
...
function TabularPanel({ workflow }: { workflow: MikeWorkflow }) {
    ...
    return (
        <div className="flex-1 border-l border-t border-border flex flex-col overflow-hidden px-3 pb-3">
            ...
            <div className="flex-1 overflow-y-auto border border-border rounded-md bg-muted/50">
                {columns.length === 0 ? (
                                            ...
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-1.5">
                                                    {col.tags.map((tag) => (
                                                        <span
                                                            key={tag}
                                                            className="inline-block rounded-full bg-accent px-2 py-0.5 text-xs text-muted-foreground font-sans"
                                                        >
                                                            {tag}
                                                        </span>
                                                    ))}
                                        ...
                                        .map((wfItem) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center gap-3 px-4 py-3 text-xs text-left border-b border-border transition-colors ${isSelected ? "bg-accent" : "hover:bg-muted/50"}`}
                                                >
                        ...
                        <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                            ...
                            <div className="px-5 pb-3 shrink-0">
                                ...
                                <textarea
                                    ...
                                    className="w-full text-sm text-foreground/80 placeholder:text-muted-foreground/70 bg-muted/50 border border-border rounded-md px-3 py-2 resize-none outline-none leading-relaxed"
                                />
                            </div>

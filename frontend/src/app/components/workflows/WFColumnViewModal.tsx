            ...
            <div className="w-full max-w-2xl rounded-2xl bg-card shadow-2xl flex flex-col h-[600px]">
                ...
                <div className="px-6 pt-3 pb-5 flex flex-col gap-4 overflow-y-auto flex-1">
                    <div>
                        ...
                        <p className="text-sm text-foreground/90">{col.name}</p>
                    </div>
                    <div>
                    ...
                    {col.tags && col.tags.length > 0 && (
                        <div>
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {col.tags.map((tag) => (
                                    <span key={tag} className="inline-block rounded-full bg-accent px-2 py-0.5 text-xs text-muted-foreground">{tag}</span>
                                ))}
                            </div>
                        </div>
                    )}
                ...
                <div className="border-t border-border/60 px-6 py-4 flex justify-end shrink-0">
                    <button onClick={onClose} className="rounded-lg bg-primary px-5 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
                        Close
                    </button>
                </div>
            </div>

...
export function WorkflowList() {
    ...
    const getTypeMeta = (type: MikeWorkflow["type"]) =>
        type === "tabular"
            ? { label: "Tabular", Icon: Table2, className: "text-primary" }
            : {
                  label: "Assistant",
                  Icon: MessageSquare,
    ...
    const typeFilterButton = (
        <div className="relative" ref={typeFilterRef}>
            ...
            {typeFilterOpen && (
                <div className="absolute right-0 top-full mt-1.5 z-20 w-40 rounded-xl border border-border/60 bg-card shadow-lg overflow-hidden">
                    <button
                        onClick={() => {
                            setTypeFilter(null);
                            setTypeFilterOpen(false);
    ...
    const practiceFilterButton = (
        <div className="relative" ref={practiceFilterRef}>
            ...
            {practiceFilterOpen && (
                <div className="absolute right-0 top-full mt-1.5 z-20 w-52 rounded-xl border border-border/60 bg-card shadow-lg overflow-hidden">
                    <button
                        onClick={() => {
        ...
        <>
            {selectedIds.length > 0 && (
                <div ref={actionsRef} className="relative">
                    ...
                    {actionsOpen && (
                        <div className="absolute top-full right-0 mt-1 w-36 rounded-lg border border-border/60 bg-card shadow-lg z-50 overflow-hidden">
                            {activeTab === "hidden" ? (
                                <button
                                    onClick={handleBulkUnhide}
                                    className="w-full px-3 py-1.5 text-left text-xs text-foreground/80 hover:bg-muted/50 transition-colors"
                                >
                                    Unhide
                                </button>
                            ) : (
                    ...
                    {loading && activeTab !== "builtin" ? (
                            ...
                            {[1, 2, 3].map((i) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0 pl-3 pr-4">
                                        <div className="h-3.5 w-48 rounded bg-accent animate-pulse" />
                                    </div>
                        ...
                        <div className="flex flex-col items-start py-24 w-full max-w-xs mx-auto">
                            {activeTab === "custom" ? (
                                <>
                                    ...
                                    <button
                                        onClick={() => setNewModalOpen(true)}
                                        className="mt-4 inline-flex items-center gap-1 rounded-full bg-primary px-3 py-1 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors shadow-md"
                                    >
                                        + Create New
                                    </button>
                                </>
                        ...
                        filtered.map((wf) => {
                            ...
                            <div
                                ...
                                className="group flex items-center h-10 pr-3 md:pr-10 border-b border-border/40 hover:bg-muted/50 cursor-pointer transition-colors"
                            >
                                <div
                                ...
                                <div className={`sticky left-8 z-[60] ${NAME_COL_W} p-2 ${rowBg} group-hover:bg-muted/50`}>

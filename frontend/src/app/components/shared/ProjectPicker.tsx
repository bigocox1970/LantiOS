            ...
            <div className="px-4 pt-1 pb-2">
                <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/50 px-3 py-2">
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent text-sm text-foreground/80 placeholder:text-muted-foreground/70 outline-none"
                        autoFocus
                    />
                    {search && (
                    ...
                    <div className="rounded-sm border border-border/60 overflow-hidden">
                        ...
                        {[65, 45, 80, 55, 70].map((w, i) => (
                            <div key={i} className="flex items-center gap-2 px-2 py-2">
                                ...
                                <div className="h-3.5 w-3.5 rounded bg-border animate-pulse shrink-0" />
                                <div className="h-3 rounded bg-border animate-pulse" style={{ width: `${w}%` }} />
                            </div>
                        ))}
                    </div>
                ) : filtered.length === 0 ? (
                            ...
                            {filtered.map((project) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-2 px-2 py-2 text-xs transition-colors text-left ${isSelected ? "bg-accent" : "hover:bg-muted/50"}`}
                                    >
                                        <span className={`shrink-0 h-3.5 w-3.5 rounded-full border flex items-center justify-center ${isSelected ? "bg-foreground border-gray-900" : "border-border"}`}>

...
interface UserProfileContextType {
    ...
    updateAssistantName: (name: string | null) => Promise<boolean>;
...
export function UserProfileProvider({ children }: { children: ReactNode }) {
    ...
    const updateAssistantName = useCallback(
        async (assistantName: string | null): Promise<boolean> => {
            if (!user) return false;
            try {
                const updated = await updateUserProfile({ assistantName });
                setProfile((prev) =>
                    prev ? { ...prev, ...toProfile(updated) } : null,
                );
                return true;

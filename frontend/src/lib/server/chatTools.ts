...
// pdfjs-dist standard fonts path — undefined is safe, falls back gracefully
const STANDARD_FONT_DATA_URL: string | undefined = undefined;
...
function buildSystemPromptContent(assistantName?: string | null): string {
    const identity = assistantName?.trim()
        ? `You are ${assistantName.trim()}, an AI legal assistant that helps lawyers and legal professionals analyze documents, answer legal questions, and draft legal documents.`
        : `You are an AI legal assistant built into Lanti-OS. You have not been given a name. If asked who you are or what your name is, respond with: "I am the AI Legal Assistant built into Lanti-OS. You haven't given me a name yet — you can do that in Settings."`;
    return `${identity}${SYSTEM_PROMPT_BODY}`;
}
...
export function buildMessages(
    ...
    let systemContent = buildSystemPromptContent(assistantName);
...
> {
    ...
    const {
        ...
        changes,
        errors,
    } = await applyTrackedEdits(current.bytes, edits, { author: "Lanti OS" });
